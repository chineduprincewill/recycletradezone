...
const SELLER_TRANSACTIONS_API = 'seller-transactions';
...
const SellerTransactions = () => {
    ...
    const fetchTransactions = async () => {
        ...
        const response = await axios.get(SELLER_TRANSACTIONS_API,
            {
                        ...
                        {transactions === null ? <Spinner /> : (transactions === undefined ? <span className="bg-warning text-white p-2 my-3">Oops! it's like your session has expired. Sign out and Sign in again.</span> :
                            (transactions.length != 0 ? transactions.map(trans => {
                                return (<div key={trans.id} className="col-xl-4">
                                    <TransactionData trans={trans} setCancelval={setCancelval} setWeightSet={setWeightSet} />
                                </div>)
                            }) : <span className="text text-danger p-3 my-3">Sorry! No transactions record found!</span>)
                        )}
                    </div>
                </div>
            </div>
