...
const SELLER_COMMISSIONS_API = 'seller-commissions';
...
const SellerCommissions = () => {
    ...
    const fetchCommissions = async () => {
        ...
        const response = await axios.get(SELLER_COMMISSIONS_API,
            {
        ...
        <div className="main-content">
            <div className="page-content">
                <div className="container-fluid">
                    ...
                    <div className="row">
                        {commissions === null ? <Spinner /> : (commissions === undefined ? <span className="bg-warning text-white p-2 my-3">Oops! it's like your session has expired. Sign out and Sign in again.</span> :
                            (commissions.length != 0 ? commissions.map(comm => {
                                return (<div key={comm.id} className="col-xl-4">
                                    <CommissionData comm={comm} />
                                </div>)
                            }) : <span className="text text-danger px-3 my-3">Sorry! No earning record found.</span>)
                        )}
                    </div>
